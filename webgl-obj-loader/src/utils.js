...
export function loadModels(model){
  const finished = [];
  const parsed = [];
  let name_mesh = model.name;
  parsed.push(Promise.resolve(name_mesh));

  parsed.push(new Mesh(model.obj));

  let material = new MaterialLibrary(model.mtl);

  parsed.push(Promise.all(Promise.resolve(material)));
